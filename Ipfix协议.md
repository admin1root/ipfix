...
##### 选项模板记录格式
和模板记录一样，选项模板记录也包含IANA分配的和/或企业特定的信息元素标识符的任意组合。
           ...
           | Field Specifier                                  |
           +--------------------------------------------------+
选项模板记录由选项模板记录头部和若干个字段说明符组成，字段说明符格式在上面已经介绍过了，选项模板记录头部格式如下：
...
字段说明：
* **Template ID**:与模板记录头部的模板id相同。

* **Field Count**:选项模板记录中的字段数，包括作用域字段。

* **Scope Field Count**:选项模板记录中的作用域数。作用域字段在收集器中生效。紧接着Scope Field Count的前N个字段为作用域字段，Scope Field Count不能为0。

下面的选项模板集包含IANA分配的混合信息和企业特定的信息元素。 它由一个设置标题，一个选项模板标题和几个字段说明符组成。
...
#### 数据记录格式
数据记录包含在数据集中被发送。它只由若干个字段值组成，字段值所属的模板ID被编码在集头部字段“Set ID”中，例如，"Set ID" = "Template ID"。格式如下：
...
请注意，字段值不一定具有16位的长度。 字段值是根据[RFC7012]中指定的数据类型编码的。
仅当在收集过程中与模板ID对应的模板记录可用时，才可以解释数据记录格式。
下面是一个包含数据记录的数据集的例子：
...
## Specific Reporting Requirements~~（具体报告要求？）~~
一些特定的选项模板和选项模板记录必须提供有关流记录和计量过程的额外信息。这可能对测定过程和输出过程的执行有一些约束。

### 测定过程统计选项模板
测定过程统计选项模板指定了测定过程统计的数据记录的结构。
它应该包含以下信息元素（定义在[IANA-IPFIX]）：
    （scope）observationDomainId
        除非封闭消息的Observation Domain ID不为零，否则该信息元素必须定义为作用域字段，并且必须存在。
    （scope）meteringProcessId
        只要存在，该字段必须被定义为作用域字段
     exportedMessageTotalCount
     exportedFlowRecordTotalCount
     exportedOctetTotalCount
输出过程应该定期或根据制定的输出策略输出测定过程统计选项模板制定的数据记录。输出周期和策略可配。
注意，如果输出器上有多个可用的测定过程，meteringProcessId 必须被指定为附加的作用域字段。
...
应该包含以下信息元素：
    ...
    time first packet ignored（时间戳格式）
    time last packet ignored（时间戳格式）
输出过程定期或按策略进行输出。输出周期和策略可配。
同输出器多测定过程情况下，meteringProcessId 必须被指定为附加的作用域字段。
收集过程对时间戳的处理是前后都是开。
